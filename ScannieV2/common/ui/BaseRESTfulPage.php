...
if (!class_exists('WebDispatch')) {
    include('WebDispatch.php');
}
class BaseRESTfulPage extends WebDispatch
{
    protected $__method = '';
    ...
    */
    ...
    {
        if (!class_exists('CoreRESTfulRouter')) {
            include('CoreRESTfulRouter.php');
        }
        if(!class_exists('FormValueContainer')) {
            include('FormValueContainer.php');
        }
        $this->routing_trait = new CoreRESTfulRouter();
        $this->form = new FormValueContainer();
        parent::__construct();
    }

...
if($_SESSION['store_id']) {
    ...
    echo "<h1 align='center'>Tag Notes</h1>";
    echo "<div align='center'>";
    if ($_SESSION['store_id'] == 1) {
        echo "<strong>Hillside</strong><br>";
    } else {
        echo "<strong>Denfeld</strong><br>";
    }
    ...
    for ($i=0; $i<count($upc); $i++) {
        if ($upc[$i] >= 0) {
            ...
            echo "<td>" . $batch[$i] . "</td>";
            echo "<td>" . $note[$i] . "</td>";
            echo "<td><button class=\"btn btn-default\" type=\"button\" onclick=\"sendToQueue(this, '{$upc[$i]}', 1, '{$_SESSION['session']}'); return false;\">Item Checked/Good</button></td>";    
            echo "<td><button class=\"btn btn-default\" type=\"button\" onclick=\"sendToQueue(this, '{$upc[$i]}', 8, '{$_SESSION['session']}'); return false;\">Missing Sign</button></tr>";  
        }
    }
    echo "</table>";
} else {
